...
export async function listarEstabelecimentos(cidade) {
  ...
  let consulta = supabase
    ...
    .select("id, nome, descricao, endereco, cidade, capa_url, aberto_agora, categorias(nome)")
    .eq("ativo", true)
    .order("nome");
...
export async function buscarMeuEstabelecimento() {
  ...
  const { data, error } = await supabase
    .from("estabelecimentos")
    .select(
      "id, nome, descricao, endereco, cidade, categoria_id, capa_url, aberto_agora, categorias(nome)"
    )
    .eq("dono_id", user.id)
    .limit(1)
    .maybeSingle();
